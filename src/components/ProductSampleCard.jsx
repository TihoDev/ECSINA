...
const ProductSampleCard = ({title="title",text="description",imageId}) => {
    return(
        <div className='w-full rounded-4xl max-h-[344px] flex flex-col shadow-[0px_2px_4px_0px_#1E132840]'>
            <div className='h-1/2'>
                {imageId ? (
                    <img src={imageId} alt="product image" className='object-cover w-full h-full rounded-tl-4xl rounded-tr-4xl'/>
                ) : (
                    <img src={notFountProductImg} alt='this product does not have image' className='object-cover w-full h-full rounded-tl-4xl rounded-tr-4xl'/>
                )}
            </div>
            <div className='p-4'>
