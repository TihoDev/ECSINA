    ...
    return(
        <AuthCardLayout>
            ...
            <div className="flex justify-between items-center">
                <p className=" flex flex-row-reverse items-center rounded-[4px] bg-gradient-to-r from-[#3E243C] via-[#71416D] via-[45.5%] to-[#A45F9E] max-w-[350px] max-h-[42px] p-3 justify-center text-center text-white font-normal text-[14px] text-nowrap">کد تایید به شماره همراه {maskPhoneNumber(phoneNumber)} ارسال شده است. </p>
                {!canResend ? (
                <p className="bg-gradient-to-r from-[#3E243C] via-[#71416D] via-[45.5%] to-[#A45F9E] bg-clip-text text-transparent text-2xl">
                {formatTime(timeLeft)}
                </p>
                ) : (
                <button onClick={handleResend} className="text-[#71416D] underline text-sm"> ارسال مجدد کد</button>)}
            </div>
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                <AuthInput placeHolder={'کد تایید'}
                    {...register("code", {
